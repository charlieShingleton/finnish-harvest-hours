...
/* app.get("/user", function(req, res) {
  if (req.isAuthenticated()) {
    ...
    User.findOne({ id: sessionUser.harvestId }, (err, doc) => {
      if (err) {
        console.error(err);
      }

      const previousBalance =
        doc && doc.previousBalance ? doc.previousBalance : 0;
      const variantPeriods =
        doc && doc.variantPeriods ? doc.variantPeriods : [];
...
function idStringToTasks(taskIds) {
  if (!taskIds) {
    ...
    return taskIds.split(",").map(taskId => {
...
app.post("/variant-periods", function(req, res) {
  if (req.isAuthenticated()) {
    const sessionUser = req.session.passport.user;
    userService.upsertUserVariantPeriods(
      sessionUser.harvestId,
      req.body.variantPeriods
    );
  }
  res.send("OK");
}); */
...
app.use(function expressHandleError(err, req, res, next) {
